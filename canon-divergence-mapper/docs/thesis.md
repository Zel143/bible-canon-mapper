...
Historical Narrative — the genre holding 4 of the 7 deuterocanonical books and 374 of the 686 deuterocanonical passages, the largest share by far — shows a within-genre JSD (0.636) *larger* than the unconditioned baseline (0.567), not smaller. Wisdom/Poetic shows a within-genre JSD (0.296) roughly half the baseline. Prophetic, resting on a single deuterocanonical book and 35 passages, falls in between (0.476). All three within-genre associations were flagged as statistically significant at p ≪ 0.001 by a chi-square test, but that test's validity was not yet checked at the time. (`notebooks/03_topic_modeling.ipynb`, `outputs/figures/topic_distribution_by_genre.png`.)

**Strengthening the Prophetic comparison.** No second deuterocanonical Prophetic-genre book exists to add — Baruch is the only one of the seven deuterocanonical books classified as Prophetic, and the Orthodox-only additions (1 Esdras, 3 Maccabees, Prayer of Manasseh, Psalm 151) are Historical Narrative or Wisdom/Poetic, not Prophetic, so extending the corpus to them would not help. Instead, two checks address the small-sample concern directly. First, Pearson's chi-square approximation breaks down when expected cell counts are small (a common threshold: unreliable once more than ~20% of cells fall below 5); checking the Prophetic contingency table directly, 11 of its 22 cells (50%) have expected counts under 5 — the chi-square p-value reported above should not be trusted at face value for this genre. Second, in place of it: a bootstrap 95% confidence interval (resampling protocanonical and deuterocanonical passages independently, with replacement, 3,000 times) and a permutation test (3,000 random reshuffles of the category label within each genre, building an empirical null distribution that doesn't depend on chi-square's large-sample assumption), applied to all three genres for consistency:
...
The permutation test is unambiguous for all three genres, including Prophetic: across 3,000 reshuffles, not one produced a JSD as large as the one actually observed (p at the floor of 1/3001 in each case) — a result that does not share the chi-square test's small-sample weakness, and that agrees with it anyway. The bootstrap CI is what "underpowered" actually looks like once quantified: Prophetic's interval is visibly wider than the other two genres' relative to its point estimate, and the honest claim is "somewhere in a wide-but-not-tiny range, and real" rather than a bare 0.476. Sample size remains a real limitation — no statistical technique manufactures Prophetic-genre deuterocanonical text that doesn't exist in any of the three canons modeled here — but the comparison is no longer resting on an untrustworthy significance test and an unquantified point estimate.
...
- **The Prophetic genre-confound result (Section 4.4) rests on one deuterocanonical book (Baruch, 35 passages) and no second one exists to add** — this is a fixed constraint of the canon, not fixable by more analysis. What's been done is proper uncertainty quantification given that constraint (bootstrap CI, permutation test valid at small n) rather than trusting an untested chi-square p-value; the bootstrap CI is wide, and that width is the honest picture, not a defect to paper over.

...
`N_TOPICS=12` (Section 3.5) was a starting point, not a tuned value. Two checks were run: a standard model-selection diagnostic (held-out perplexity across candidate topic counts), and — more directly relevant to this thesis's actual claim — whether the Historical Narrative genre-confound result (Section 4.4) holds up under different topic counts and random initializations, or is an artifact of the specific `K=12, random_state=42` used throughout.

**K sweep** (fit on an 80% training split, held-out perplexity on the remaining 20%, then the same trained model applied to the full dataset to recheck the Historical Narrative finding):

| K | Held-out perplexity | Baseline JSD | Historical Narrative JSD | Historical Narrative ≥ baseline? |
|---|---|---|---|---|
| 8 | **774.6** (best) | 0.407 | 0.478 | Yes |
| 12 | 810.2 | 0.551 | 0.610 | Yes |
| 16 | 876.4 | 0.476 | 0.454 | No (narrowly) |
| 20 | 944.3 | 0.538 | 0.607 | Yes |

Held-out perplexity increases monotonically from K=8 through K=20 in this grid — by that metric alone, K=8 fits better than K=12. The Historical Narrative finding holds at 3 of the 4 K values tested, failing only at K=16, and even there the shortfall is narrow (0.454 vs. 0.476) rather than a reversal.
...
- **The topic count (12) is not perplexity-optimal.** Section 4.5's held-out perplexity sweep favors K=8 over K=12 in the range tested (8, 12, 16, 20). K=12 was kept because switching would require redoing the topic-word interpretation in Section 4.3 for a different topic count, and because the more directly relevant check — whether the Historical Narrative genre-confound finding holds up — passes at K=12 across every random seed tested and at 3 of the 4 topic counts tested (Section 4.5). K=8 remains a reasonable, documented alternative for future work rather than one adopted here.
- **The perplexity sweep only covers K ∈ {8, 12, 16, 20}.** A finer or wider grid, or a proper coherence metric (e.g. via an external topic-coherence library, not attempted here to avoid another dependency), might identify a better-fitting K than either 8 or 12.
